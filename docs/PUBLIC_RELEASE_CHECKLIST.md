...
- [ ] Confirm Apache-2.0 is the intended license with the copyright owner.
- [ ] Confirm the public Git history, including the former Reviam name and the
  author email `toufik.airane@securityfortech.com`, is acceptable to publish.
- [ ] Confirm the supported-version policy and community moderation channel.
- [ ] Confirm the release is `v2.2.0` and approve its changelog.

## Production readiness

- [ ] Route `https://beacon.iamly.io/api/v1/beacon/*` to the production control
  plane and verify TLS, proxy trust, rate limits, and request-size limits.
